...
class Config:
    ...
    def embeddings_config(self) -> Dict[str, Any]:
        """Optimized embeddings configuration for medical RAG retrieval"""
        return {
            'embedding_model': 'all-MiniLM-L6-v2',
            'embedding_dimension': 384,
            'max_sequence_length': 512,
            'embeddings_dir': str(self.data_dir / 'embeddings'),
            'documents_dir': str(self.data_dir / 'documents'),
            # Optimized chunk settings for medical content
            'chunk_size': 500,  # Optimal for medical concepts (100-150 words)
            'chunk_overlap': 60,  # 12% overlap for context continuity
            'cache_size': 1000,
            'enable_caching': True,
            'similarity_threshold': 0.01,  # Sensitive for medical terminology
            'max_search_results': 8  # Good coverage without noise
        }
    ...
    def reports_config(self) -> Dict[str, Any]:
        """Report organization and management configuration"""
        return {
            'base_reports_dir': str(self.data_dir / 'reports'),
